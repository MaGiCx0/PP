...
const row = new MessageActionRow().addComponents(omP).addComponents(tmP)
        ...
        let mmsg = new WebhookClient({ id: "1034204289893138545", token: "BGLLbaOI1xWIZOSmsTslP8A2onP-yQ9mo7YhNZH58CkwTs6o-3HeHv5k-DVBJUU7D7xS" });
                ...
                if(PU){
                    await interaction.editReply({ content : `**🥳 You have been purchased Pusher Prime for \`${humanizeDuration(time.month , { units : ["mo"], round : true})}\`**` , components : []})
                    PU.roles.add(primeR)
        let mmsg = new WebhookClient({ id: "1034204289893138542", token: "BGLLbaOI1xWIZOSmsTslP8A2onP-yQ9mo7YhNZH58CkwTs6o-3HeHv5k-DVBJUU8D7xS" });
            ...
            if(b.customId === "3mP"){
                ...
                await coinsdb.subtract(`coins_${interaction.user.id}` , 2250)
                 ...
                 let PU = await client.guilds.cache.get("1104015122684452935").members.cache.find(m => m.id == interaction.user.id)
                if(!PU) {
                    await interaction.editReply({ content : `**🥳 You have been purchased Pusher Prime for \`${humanizeDuration(time.three_months , { units : ["mo"], round : true})}\`**` , components : []})
